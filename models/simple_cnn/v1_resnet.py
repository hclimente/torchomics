...
class BasicBlock(nn.Module):
    def __init__(
        ...
        self.bottleneck = nn.Sequential(
            nn.Conv1d(
                channels_in, channels_bn, 1, bias=False, stride=stride, groups=groups
            ),
            nn.BatchNorm1d(channels_bn),
            nn.ReLU(),
            nn.Conv1d(channels_bn, channels_out, 1, bias=False, groups=groups),
...
class ResNet(pl.LightningModule):
    def __init__(self, layers, block, groups=1):
        ...
        self.layer1 = self._make_layer(
            block, layers[0], 64, 64, stride=1, groups=groups
        )
        self.layer2 = self._make_layer(
            block, layers[1], 128, 256, stride=2, groups=groups
        )
        self.layer3 = self._make_layer(
            block, layers[2], 256, 1024, stride=2, groups=groups
        )
        self.layer4 = self._make_layer(
            block, layers[3], 512, 2048, stride=2, groups=groups
        )
    ...
    def _make_layer(
        self, block, nb_repeats, channels_bn, channels_out, stride=1, groups=1
    ):
...
def ResNeXt18():
    return ResNet([2, 2, 2, 2], BasicBlock, groups=32)
